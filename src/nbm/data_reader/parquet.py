...
load_dotenv(Path(__file__).parents[3] / "data" / "raw" / "parquet-keys.env")

try:
    PK = os.environ["PK"]
    PK2 = os.environ["PK2"]
except KeyError:
    print("PK and PK2 not found in environment variables")
    PK = ""
    PK2 = ""

KMS_CC = pe.KmsConnectionConfig(custom_kms_conf={"footer": PK, "columns": PK})

KMS_CC2 = pe.KmsConnectionConfig(custom_kms_conf={"footer": PK2, "columns": PK2})


def read(
    in_file: str, usecols: List[str] = [], index_cols: List[str] = []
) -> pd.DataFrame:

    try:
        cripto_factory = pe.CryptoFactory(InMemoryKmsClient)
        decrypt_prop = cripto_factory.file_decryption_properties(KMS_CC)
        parquet_file_data = pq.ParquetFile(in_file, decryption_properties=decrypt_prop)
    except ValueError:
        cripto_factory = pe.CryptoFactory(InMemoryKmsClient)
