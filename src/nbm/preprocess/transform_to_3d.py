from pathlib import Path

from deprecated import deprecated
import h5py
import numpy as np
import pandas as pd

from data_reader.data import listTurbines
from data_reader.data import readTurbine
from preprocess.cls_indexer import Indexer


_REMOVED_COLS = [  # columns that are for sure not needed
    "turbineid",
    "site",
    "phase",
    "manufacturer",
    "model",
]

_FLAG_COLS = [  # these columns are flags and should not be modified
    "turbulentvalid",
    "underperformanceprobabilityvalid",
    "overperformanceprobabilityvalid",
]

ORG_FEAT_COUNT = "original_feature_count"
VALID_COL = "underperformanceprobabilityvalid"

# default values for getStackedTurbineData
DEF_N_STEPS = 16
DEF_SHIFT = 1
DEF_SAMPLE_LEN_S = 60 * 60  # default to 60 minutes -> 6 rows per sample


def resampleTurbineData(dfData: pd.DataFrame, sampleLen_s: int) -> pd.DataFrame:
    if "turbulent" in dfData.columns:
        dfData["turbulent"] = dfData["turbulent"].astype(float)

    # resample data
    dfData["datetime"] = pd.to_datetime(dfData["datetime"])
    dfData.set_index("datetime", inplace=True)
    dfData.drop(
        columns=[col for col in _REMOVED_COLS if (col in dfData.columns)],
        inplace=True,
    )
    dfFilled = dfData.resample(f"{sampleLen_s}s").mean()

    # mark empty rows
    dfFilled[ORG_FEAT_COUNT] = dfFilled.notna().astype(int).sum(axis=1)

    # empty out invalid data, except for flag columns
    emptyingCols = dfFilled.columns.difference(_FLAG_COLS).difference([ORG_FEAT_COUNT])
    dfFilled.loc[dfFilled[VALID_COL] == 0, emptyingCols] = np.nan
    # set n_features to 0 if not valid
    dfFilled.loc[dfFilled[VALID_COL] == 0, ORG_FEAT_COUNT] = 0

    dfFilled.interpolate(method="time", inplace=True)

    dfFilled.reset_index(inplace=True)
    dfFilled.rename(columns={"index": "datetime"}, inplace=True)

    # convert date to numpy float
    dfFilled["datetime"] = (
        dfFilled["datetime"].astype(np.int64) // 10**9  # nanoseconds to seconds
    )

    return dfFilled


def generateStackedTurbineData(
    name: str,
    n_timesteps=DEF_N_STEPS,
    shift=DEF_SHIFT,
    stepLen_s=DEF_SAMPLE_LEN_S,
    conditions=[],
    dataPath=None,
    verbose=False,
) -> tuple[Indexer, list[np.ndarray]]:
    """
    Generate stacked turbine data for a given turbine name.

    Args:
        name (str): Name of the turbine.
        n_timesteps (int): Number of timesteps per sample.
        shift (int): Number of different rows between 2 consecutive samples.
        stepLen_s (int): Length of each sample in seconds.
        conditions (list): List of conditions to filter the samples. A condition is a function that takes a 2d array (sample) and returns True if the sample meets the condition.
        verbose (bool): Whether to print verbose output.

    Returns:
        tuple[Indexer, list[np.ndarray]]: A tuple containing the column names and a list of 3d arrays containing the stacked turbine data based on the conditions.
    """

    dfData = readTurbine(name, dataPath=dataPath)

    dfFilled = resampleTurbineData(dfData, stepLen_s)

    if verbose:
        originalLen = len(dfData)
        originalNans = dfData.isna().sum().to_dict()

        n_nan = dfFilled.isna().sum().to_dict()

        print(f"Length of data: {len(dfFilled)} / {originalLen}")
        print("Nans in features:")
        for col in n_nan.keys() & originalNans.keys():
            print(f"\t{col}: {n_nan[col]} / {originalNans[col]}")

        print("=" * 50)

    # ID_OF_VALID_COL = dfFilled.columns.get_loc(VALID_COL)
    # ID_OF_N_FEAT = dfFilled.columns.get_loc(ORG_FEAT_COUNT)
    
    out = [[] for _ in range(len(conditions))]
    
    for i in range(0, len(dfFilled) - n_timesteps, shift):
        sample = dfFilled.iloc[i : i + n_timesteps]
        if sample[ORG_FEAT_COUNT].sum() <= 0 and sample[VALID_COL].sum() <= 0:
            continue  # skip samples with no valid data

        for i, condition in enumerate(conditions):
            if condition(sample):
                out[i].append(sample.to_numpy(dtype=np.float32).T)  # transpose to (n_features, n_timesteps)
                
    return Indexer(tags=list(dfFilled.columns), defaultDim=1), [np.array(elem) for elem in out]


@deprecated
def getStackedTurbineData(
    name: str,
    n_steps=DEF_N_STEPS,
    shift=DEF_SHIFT,
    sampleLen_s=DEF_SAMPLE_LEN_S,
    recompute=False,
    verbose=False,
) -> tuple[pd.Index, h5py.Dataset, h5py.File]:
    DSET_ID = "data3d"
    COLS_ID = "columns"

    ## >>> read cached data
    dataFile = Path("tmp") / f"dataset_{name}_{n_steps}_{shift}_{sampleLen_s}.hdf5"
    if recompute:
        if dataFile.exists():
            dataFile.unlink()

    if dataFile.exists():
        f = h5py.File(dataFile, "r", swmr=True)
        if DSET_ID in f and COLS_ID in f:
            colStore = f[COLS_ID]
            if isinstance(colStore, h5py.Dataset):
                cols = pd.Index(colStore.asstr()[()])
            else:
                f.close()
                raise KeyError(
                    f"Columns' storage ``{name}`` exists but is not a h5py.Dataset"
                )

            dset = f[DSET_ID]
            if isinstance(dset, h5py.Dataset):
                return cols, dset, f
            else:
                f.close()
                raise KeyError(f"Dataset ``{name}`` exists but is not a h5py.Dataset")
        else:
            f.close()
    ## <<< read cached data

    dfData = readTurbine(name)

    if verbose:
        originalLen = len(dfData)
        originalNans = dfData.isna().sum().to_dict()

    dfFilled = resampleTurbineData(dfData, sampleLen_s)

    if verbose:
        n_nan = dfFilled.isna().sum().to_dict()

        print(f"Length of data: {len(dfFilled)} / {originalLen}")
        print("Nans in features:")
        for col in n_nan.keys() & originalNans.keys():
            print(f"\t{col}: {n_nan[col]} / {originalNans[col]}")

        print("=" * 50)

    # stack data
    N_ROWS_PER_DAY = int(24 * 60 * 60 / sampleLen_s)  # 24h in seconds / sampleLen_s

    timeSteps = n_steps * N_ROWS_PER_DAY

    arrayFilled = dfFilled.to_numpy()
    arrayFilled = arrayFilled.astype(np.float32)

    n_stack = (len(arrayFilled) - timeSteps) // shift

    dataFile.parent.mkdir(parents=True, exist_ok=True)
    f = h5py.File(dataFile, "a")
    dset = f.create_dataset(
        DSET_ID,
        (n_stack, arrayFilled.shape[1], timeSteps),
        dtype=np.float32,
        chunks=True,
    )

    ID_OF_VALID_COL = dfFilled.columns.get_loc(VALID_COL)
    ID_OF_N_FEAT = dfFilled.columns.get_loc(ORG_FEAT_COUNT)

    j = 0
    for i in range(n_stack):
        sample = arrayFilled[i * shift : i * shift + timeSteps]

        # check if sample has at least one valid value
        if sample[:, ID_OF_N_FEAT].sum() > 0 and sample[:, ID_OF_VALID_COL].sum() > 0:
            dset[j] = sample.T
            j += 1

    # resize dataset to actual size
    dset.resize((j, arrayFilled.shape[1], timeSteps))

    f.create_dataset(COLS_ID, data=dfFilled.columns.to_numpy(dtype="S"))

    return dfFilled.columns, dset, f


@deprecated
class TurbineData:
    USING_TURBINES = set()
    COL_UNDERPERF_PROBA = "underperformanceprobability"
    COL_UNDERPERF_VALID = "underperformanceprobabilityvalid"
    COL_NORMAL = "normalbehaviour"

    def __init__(
        self,
        turbineName: str,
        n_steps=DEF_N_STEPS,
        shift=DEF_SHIFT,
        sampleLen_s=DEF_SAMPLE_LEN_S,
        verbose=False,
    ):
        self.turbineName = turbineName
        self.verbose = verbose

        if verbose:
            if turbineName in TurbineData.USING_TURBINES:
                print(f"Warning: {turbineName} is already in use")
        TurbineData.USING_TURBINES.add(turbineName)

        self.columns, self.data3d, self.f = getStackedTurbineData(
            turbineName, n_steps, shift, sampleLen_s, verbose=verbose
        )

        self._idUnderperfProba = self.getIdOfColumn(self.COL_UNDERPERF_PROBA)
        self._idUnderperfValid = self.getIdOfColumn(self.COL_UNDERPERF_VALID)
        self._idN_OrgFeat = self.getIdOfColumn(ORG_FEAT_COUNT)
        self._idNormal = self.getIdOfColumn(self.COL_NORMAL)

    def getIdOfColumn(self, columnName: str) -> int:
        """
        Get the index of a column in the dataset

        Args:
            columnName (str): name of the column

        Returns:
            int: index of the column

        Raises:
            KeyError: if the column does not exist
        """
        return self.columns.get_loc(columnName)  # type: ignore # overload's bug

    def _evalUnderperformValid(
        self, turbineData2d, maxConsecutiveInvalid, maxInvalidRate
    ):
        nContinuousInvalid = 0
        nInvalid = 0

        for timeStep in turbineData2d.T:
            if (
                (timeStep[self._idUnderperfValid] == 0)
                or (timeStep[self._idUnderperfValid] == np.nan)
                or (timeStep[self._idN_OrgFeat] == 0)
            ):
                nContinuousInvalid += 1
                nInvalid += 1

                if nContinuousInvalid > maxConsecutiveInvalid:
                    return False
                if nInvalid > maxInvalidRate * len(turbineData2d):
                    return False

            else:
                nContinuousInvalid = 0

        # check if last value is valid
        if (
            (timeStep[self._idUnderperfValid] == 0)
            or (timeStep[self._idUnderperfValid] == np.nan)
            or (timeStep[self._idN_OrgFeat] == 0)
        ):
            return False

        return True

    def _evalUnderperformProba(
        self,
        turbineData2d,
        underperformThreshold,
        maxConsecutiveUnderperform,
        maxUnderperformRate,
    ):
        if underperformThreshold >= 1:
            return True

        nContinuousUnderperform = 0
        nUnderperform = 0

        for timeStep in turbineData2d.T:
            if (
                (timeStep[self._idUnderperfProba] > underperformThreshold)
                or (timeStep[self._idUnderperfProba] == np.nan)
                or (timeStep[self._idN_OrgFeat] == 0)
            ):
                nContinuousUnderperform += 1
                nUnderperform += 1

                if nContinuousUnderperform > maxConsecutiveUnderperform:
                    return False
                if nUnderperform > maxUnderperformRate * len(turbineData2d):
                    return False

            else:
                nContinuousUnderperform = 0
        return True

    def _evalNormal(
        self,
        turbineData2d,
    ):
        # check if all the value at idNormal == True (1)
        target = turbineData2d[self._idNormal].astype(bool)
        return target.sum() >= np.floor(len(target) * 0.95)

    def evalUnderperformValid(
        self,
        maxConsecutiveInvalid=3,
        maxInvalidRate=0.1,
    ):
        """
        Evaluate if the turbine data is valid under the underperformance criteria.

        Args:
            maxConsecutiveInvalid (int): maximum number of consecutive invalid values
            maxInvalidRate (float): maximum rate of invalid values

        Returns:
            bool: True if the turbine data is valid, False otherwise
        """

        validness = []
        for i in range(self.data3d.shape[0]):
            validness.append(
                self._evalUnderperformValid(
                    self.data3d[i],
                    maxConsecutiveInvalid,
                    maxInvalidRate,
                )
            )
        validness = np.array(validness)
        return validness

    def evalNormal(self):
        """
        Evaluate if the turbine data is normal under the normal behaviour criteria.

        Returns:
            bool: True if the turbine data is normal, False otherwise
        """

        normalness = []
        for i in range(self.data3d.shape[0]):
            normalness.append(self._evalNormal(self.data3d[i]))
        normalness = np.array(normalness)
        return normalness

    def getNormalIndices(
        self,
        maxConsecutiveInvalid=3,
        maxInvalidRate=0.1,
        underperformThreshold=0.7,
        maxConsecutiveUnderperform=3,
        maxUnderperformRate=0.1,
        evalNormal=False,
    ):
        """DEPRECATED: Use `eval*` methods instead."""
        validness = []
        for i in range(self.data3d.shape[0]):
            validness.append(
                self._evalUnderperformValid(
                    self.data3d[i],
                    maxConsecutiveInvalid,
                    maxInvalidRate,
                )
            )
        validness = np.array(validness)
        if self.verbose:
            print(f"Valid: {sum(validness)}")

        notUnderperformness = []
        for i in range(self.data3d.shape[0]):
            notUnderperformness.append(
                self._evalUnderperformProba(
                    self.data3d[i],
                    underperformThreshold,
                    maxConsecutiveUnderperform,
                    maxUnderperformRate,
                )
            )
        notUnderperformness = np.array(notUnderperformness)
        if self.verbose:
            print(f"Not underperform: {sum(notUnderperformness)}")

        if evalNormal:
            normalness = []
            for i in range(self.data3d.shape[0]):
                normalness.append(self._evalNormal(self.data3d[i]))
            normalness = np.array(normalness)
            if self.verbose:
                print(f"Normal Behaviour: {sum(normalness)}")
        else:
            normalness = np.ones(self.data3d.shape[0], dtype=bool)

        normalIndices = np.where(validness & notUnderperformness & normalness)[0]
        if self.verbose:
            print(f"Normal: {len(normalIndices)}")

        return normalIndices

    def close(self):
        self.f.close()
        try:
            TurbineData.USING_TURBINES.remove(self.turbineName)
        except KeyError:
            if self.verbose:
                print(
                    f"Warning: Data tracking is broken, {self.turbineName} is not tracked when closing"
                )

    def __del__(self):
        self.close()


class TurbineDataCompressed:

    def __init__(
        self,
        turbineName: str,
        n_days=DEF_N_STEPS,
        shift=DEF_SHIFT,
        sampleLen_s=DEF_SAMPLE_LEN_S,
        verbose=False,
    ):
        self.turbineName = turbineName
        self.verbose = verbose

        if verbose:
            if turbineName in TurbineData.USING_TURBINES:
                print(f"Warning: {turbineName} is already in use")
        TurbineData.USING_TURBINES.add(turbineName)

        self.columns, self.data3d, self.f = getStackedTurbineData(
            turbineName, n_days, shift, sampleLen_s, verbose=verbose
        )


if __name__ == "__main__":
    for turbine in listTurbines():
        print(f"Reading {turbine}")
        _, dset, f = getStackedTurbineData(turbine, recompute=True, verbose=True)
        print(dset.shape)
        f.close()
